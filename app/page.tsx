...
function mergeStoredDays(stored: TripDay[] | null) {
  ...
  return tripDays.map((baseDay) => {
    ...
    const items = savedItems.map((savedItem) => {
      ...
      if (baseItem.info) mergedItem.info = { ...mergedItem.info, ...baseItem.info, links: baseItem.info.links };
      return mergedItem;
    });
...
function ScheduleView({ days, selectedDate, selectedDay, expenses, fieldNotes, onSelectDate, onEdit, onInfo, onExpense, onFieldNote, onReorder, onAdd }: { days: TripDay[]; selectedDate: string; selectedDay: TripDay; expenses: Expense[]; fieldNotes: FieldNote[]; onSelectDate: (date: string) => void; onEdit: (item: ScheduleItem) => void; onInfo: (item: ScheduleItem) => void; onExpense: (date: string, item: ScheduleItem) => void; onFieldNote: (date: string, item: ScheduleItem) => void; onReorder: (activeId: string, overId: string) => void; onAdd: () => void }) {
  ...
  return <>
    ...
    <section className="day-panel">
      <div className="day-panel-top"><div><div className="day-heading-meta"><span className="day-number">DAY {String(days.findIndex((day) => day.date === selectedDay.date) + 1).padStart(2, "0")}</span><span className="date-label">{selectedDay.weekday} · {formatDate(selectedDay.date)} · {selectedDay.area}</span></div><h3>{selectedDay.title}</h3><p>{selectedDay.summary}</p>{selectedDay.summaryLink && <a className="day-summary-link" href={selectedDay.summaryLink.href} target="_blank" rel="noreferrer">{selectedDay.summaryLink.label} ↗</a>}</div><button className="add-button" onClick={onAdd}>＋ 新增行程</button></div>
